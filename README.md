...
| <a name="module_cos_instance"></a> [cos\_instance](#module\_cos\_instance) | terraform-ibm-modules/cos/ibm | 8.19.5 |
...
| <a name="input_enable_ocp_console"></a> [enable\_ocp\_console](#input\_enable\_ocp\_console) | Flag to specify whether to enable or disable the OpenShift console. If deploying a private cluster from a machine that does not have access to private endpoints, set this as `null` to avoid failures. Allowed values are `true` to enable the console, `false` to disable the console and `null` to retain the current setting unless explicitly set to `true` or `false`. | `bool` | `null` | no |
