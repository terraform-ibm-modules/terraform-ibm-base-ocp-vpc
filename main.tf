...
locals {
  ...
  worker_pool_rhcos_entry   = [for worker in var.worker_pools : (worker.operating_system == local.os_rhel || (worker.operating_system == null || worker.operating_system == local.os_rhcos && local.is_valid_version) ? true : false)]
...
resource "ibm_container_vpc_worker_pool" "pool" {
  ...
  operating_system  = each.value.pool_name == "default" ? var.operating_system : each.value.operating_system
