...
## Note: If no Context Based Restriction(CBR) rules are passed, you must configure Context Based Restrictions externally or an IP allow list to be compliant.
...
| <a name="input_kms_config"></a> [kms\_config](#input\_kms\_config) | Use to attach a HPCS instance to the cluster | <pre>object({<br>    crk_id           = string<br>    instance_id      = string<br>    private_endpoint = optional(bool, true) # defaults to true<br>  })</pre> | n/a | yes |
...
| <a name="input_skip_iam_authorization_policy"></a> [skip\_iam\_authorization\_policy](#input\_skip\_iam\_authorization\_policy) | Set to true to skip the creation of an IAM authorization policy that permits the vpc instance  to use the encryption key from the KMS instance. | `bool` | `false` | no |
