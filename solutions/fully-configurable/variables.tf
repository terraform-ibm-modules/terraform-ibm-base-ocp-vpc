...
variable "existing_kms_instance_crn" {
  type        = string
  default     = null
  description = "The CRN of an existing Key Protect instance. If the KMS instance is in different account you must also provide a value for `ibmcloud_kms_api_key`."
...
variable "existing_cluster_kms_key_crn" {
  ...
  validation {
    condition = anytrue([
      can(regex("^crn:v\\d:(.*:){2}kms:(.*:)([aos]\\/[\\w_\\-]+):[0-9a-fA-F]{8}(?:-[0-9a-fA-F]{4}){3}-[0-9a-fA-F]{12}:key:[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$", var.existing_cluster_kms_key_crn)),
