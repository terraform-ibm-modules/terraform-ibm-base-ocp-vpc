...
variable "existing_cluster_kms_key_crn" {
  ...
  validation {
    condition     = var.existing_cluster_kms_key_crn != null || var.existing_kms_instance_crn != null
    error_message = "A value must be passed for either 'existing_cluster_kms_key_crn' or 'existing_kms_instance_crn'"
  }
...
variable "existing_boot_volume_kms_key_crn" {
  ...
  validation {
    condition     = var.existing_boot_volume_kms_key_crn != null ? var.existing_kms_instance_crn == null : true
    error_message = "A value should not be passed for 'existing_kms_instance_crn' when passing an existing key value using the 'existing_boot_volume_kms_key_crn' input."
  }

  validation {
    condition     = var.existing_boot_volume_kms_key_crn != null || var.existing_kms_instance_crn != null
    error_message = "A value must be passed for either 'existing_boot_volume_kms_key_crn' or 'existing_kms_instance_crn'"
  }
}
